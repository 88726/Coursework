...
//The method has to be public in order to allow interaction with the Jersey library
    public String list() {
        ...
        try {
            PreparedStatement ps = Server.Main.db.prepareStatement("SELECT studentID, studentName, studentSurnameInitial, studentPassword, parentID, token, totalPoints FROM students");
            ...
            while (results.next()) {
                ...
                String token = results.getString(6);
                int totalPoints = results.getInt(7);
                System.out.println(studentID + " " + studentName + " " + studentSurnameInitial + " " + studentPassword + " " + parentID + " " + token + " " + totalPoints);
                ...
                item.put("token", results.getString(6));
                item.put("totalPoints", results.getInt(7));
                list.add(item);
...
//The method has to be public in order to allow interaction with the Jersey library
    public String updateBackground(@FormDataParam("backgroundID") String backgroundID, @FormDataParam("studentID") String studentID) {

        try {

            if (studentID == null || backgroundID == null) {
                throw new Exception("One or more form data parameters are missing in the HTTP request.");
            }
            System.out.println("student/updateBackground backgroundID=" + backgroundID);
            PreparedStatement ps = Server.Main.db.prepareStatement("UPDATE students SET backgroundID = ? WHERE studentID = ?");
...
//The method has to be public in order to allow interaction with the Jersey library
    public String updatePoints(@CookieParam("token") String token, @FormDataParam("studentID") String studentID, @FormDataParam("totalPoints") String totalPoints ) {
        if (!studentControl.validToken(token)) {
            return "{\"error\": \"You don't appear to be logged in.\"}";
        }
        ...
        try {
            ...
            PreparedStatement ps = Server.Main.db.prepareStatement("UPDATE students SET totalPoints = ? where studentID = ? ");

            ps.setString(1, totalPoints);
            ps.setString(2, studentID);
...
//The method has to be public in order to allow interaction with the Jersey library
    public String get(@PathParam("token") Integer token) {
        ...
        System.out.println("student/get/" + token);
        JSONObject item = new JSONObject();
        ...
        try {
            ...
            if (results.next()) {
                // item.put("id", token);
                item.put("studentID", results.getString(1));
